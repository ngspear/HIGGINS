...
def plotstft(audiopath, binsize=2**10, plotpath=None, colormap="jet"):
    ...
    #np.savetxt(path.join(path.dirname(audiopath), 'samplerateArray.txt'), samplerate)
    ...
    with open("testforgarrett.txt", "a") as f:
        for i in range(0,len(freq)):
            f.write(str(((i*len(samples)/timebins)+(0.5*binsize))/samplerate))
